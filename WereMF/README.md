...
```json
{
    ...
    "shiwu_kidnapped": false, // 当前是否被至少一名实物绑架
...
| `jiaohua_vote_block_broadcast` | 脚滑人禁票广播；选择后再次广播完整施加关系 | `甲可以禁票一人` / `乙被甲禁票` |
| `jiaohua_dead_skill_broadcast` | 脚滑人死亡技能广播；选择后再次广播完整施加关系 | `甲可以取消…` / `乙被已死亡的甲封住…` / `乙被已死亡的甲保护` |
...
其中 `invalid_vote` 是该玩家不能投票的目标 id 列表。
