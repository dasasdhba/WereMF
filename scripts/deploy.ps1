...
if ([string]::IsNullOrWhiteSpace($ConfigPath)) { $ConfigPath = Join-Path $repoRoot "WereMF/bin/Release/net8.0/win-x64/publish/config.json" }
...
archive='$remoteArchive'; session='$TmuxSession'; port='$Port'
...
start_server() { local target="`$1" game_path="./game/WereMF"; [ -x "`$target/game/WereMF" ] || game_path="./WereMF"; tmux new-session -d -s "`$session" "cd '`$target' && exec ./WereMFServer --path `$game_path --config ./config.json --host 0.0.0.0 --port `$port$debugArg"; }
