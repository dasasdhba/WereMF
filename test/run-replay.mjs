...
const serverArgs = ["--path", gameExe, "--host", "127.0.0.1", "--port", String(port), "--disable-llm-bots", "--request-timeout-seconds", "1", "--vote-seconds-per-alive", "1", "--vote-penalty-seconds", "0"];
...
try {
  for (let i = 0; i < 100; i++) {
    if (serverOutput.includes("Now listening")) break;
