...
- `game_update_night`/`game_update_day` 会替换完整实体快照；`game_update_night_patch` 只由 `applyEntityStatePatch` 按 `player_id` 合并消息中出现的公开状态字段，未知玩家、`role`/`player` 或非法字段会被忽略，不会清除已有身份和其他状态。
- myz 威胁会清除被威胁技能的旧预选/预提交并显示醒目说明：普通威胁要求玩家重新决定，强制威胁则只展示 CLI 仍允许选择的附加效果（例如 Doge 是否自爆）。
