...
- [ ] 断线重连按顺序重放完整快照和后续 patch，最终 UI 状态与未断线客户端一致。
...
- [ ] `HuiKaSkill.ExecuteQueued` 不再发送完整 `GameContext`。
- [ ] 夜间烟雾 UI 能即时更新。
- [ ] 任何与本次烟雾无关的夜间状态都不能通过 patch 被观察到。
- [ ] CLI 文本日志、技能顺序和随机数消费顺序不变。
- [ ] Server/Web README 与 F# README 对 patch 语义描述一致。
- [ ] 对应回归测试可在无人值守环境中重复运行。
...
- [ ] 更新三个子项目 README 中的职责边界图。
- [ ] 在 F# README 中列出完整快照 API 与增量 patch API 的区别。
- [ ] 在 Server README 中记录每类 CLI 消息的路由、历史和重连规则。
- [ ] 在 Web README 中记录 reducer、effects 和 patch 合并语义。
- [ ] 清理已经不再使用的 `showMyzThreaten` 特例前，先证明 night patch 不依赖该清理。
- [ ] 只有在所有调用点和回放测试通过后，才删除旧兼容代码。
- [ ] 将临时诊断日志、运行输出和工具文件与可重复测试资产区分目录，避免误提交。
...
- [ ] 灰卡比夜间不再发送完整游戏状态。
- [ ] myz 或未来高优先级技能的隐藏状态不会因烟雾 UI 更新而暴露。
- [ ] 烟雾计数及烟雾直接造成的公开连锁结果能即时显示。
- [ ] F# CLI 的规则行为、随机数消费、输入格式和现有日志保持兼容。
- [ ] Server 仍是规则无关的进程协调和联网层。
- [ ] Web 仍是展示和输入组织层，不自行推导结算结果。
- [ ] 匿名、重连、预提交、投票、Bot、LLM 回退和日志下载均通过回归测试。
- [ ] 真实长对局回放结果与重构前一致。
