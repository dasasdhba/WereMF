...
internal sealed record BotSpeechContext(int PlayerId, string PlayerName, string Trigger, string VisibleContext, string VoteContext, string RuleFocus = "", BotSpeechResponseMode ResponseMode = BotSpeechResponseMode.Optional)
{
    public string ToPrompt() => string.Join('\n',
        ...
        "",
        $"回应模式：{ResponseMode}。",
        "先判断是否有实际信息、推导、明确被推出风险下的必要自保、被点名回应、拉票或必要干扰价值；有则把想让全场看到的内容写进 text，无则选择 silent。text 会原样广播给所有玩家，是公开聊天消息，不是给系统看的行动理由、思维链、行动解释或队友指令；不想公开的理由不要写进去。投票本身已经表达立场，不能仅因为自己投票或收到少量票就解释。",
        ...
        "再决定是否现在投票：投票阶段有合法非零目标时通常应立即投一票。第一票尚未使用且场上连续一个思考间隔无人发言、无人投票时，即使没有充分依据也应随机选择合法玩家，避免半数弃票风险；预算不超过 60 秒或只剩最后一次机会时也不得继续观望。vote 是你自己的动作；vote 等于自己时，只表示你提交了自投，不能自动推出 text 应该请求别人投你。是否公开请求别人投自己，必须另按身份规则、胜利条件和公开收益判断。只能选择提供的合法票值，已无票或尚未开始投票时必须返回 null。",
