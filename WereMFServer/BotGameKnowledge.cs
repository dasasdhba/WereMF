...
internal static class BotGameKnowledge
{
    ...
    public const string Strategy = """
        ...
        - 普通开局重抽身份机会由服务器 Bot 策略固定放弃，Bot 不得主动触发 request_reroll_player；叶子第一身份的独立重抽请求仍可按叶子规则处理。
        ...
        - 投票目标必须因为阵营嫌疑或对胜负的实际威胁被选择；身份特殊、信息量大、自爆、沉默、被点名都不能单独构成投票理由。
        - 被实物绑架只表示夜间技能受限；被 myz 威胁只表示白天无法行动。两者都不增加目标属于敌方的概率，不得因为其无法行动、无法投票或缺少技能结果而集火。
        - 公开身份声明不是服务器确认。自称吧方身份只能算弱阵营证据；自称吧主因该身份按规则应保密而可信度下降，但仍不得仅凭声明投票。身份声明必须结合技能结果、公开矛盾、票型和胜负收益判断。
        - 如果己方在人数上占优，优先寻找敌方目标，并提高误杀高可信队友或关键身份的投票门槛；不要为了快速形成票型而投唯一有信息量的玩家。只有服务器私密信息或多个独立公开事实确认的队友，才可视为已知队友。
        - 第一票在没有可靠信息时可以选择合法非零目标以避免弃票随机驱逐；自投是合法的防弃票动作，不代表自己属于敌方。第二票只有在新证据改变判断、当前目标失去嫌疑或临近截止需要跟随明确票型时才改票。严格区分字段语义：JSON 的 text 是会原样广播给全场的公开聊天消息，不是给系统看的投票理由、思维链、行动解释或队友指令；只有确实想让其他玩家看到的内容才放进 text，不想公开就保持 text 为空并选择 silent。vote 是你自己提交的票，vote="2" 只表示你把自己的票投给 2 号，不表示让别人投 2 号。自投与是否在 text 中请求别人投自己是两个独立决策；不要因为自投合法就自动生成“请投我”，也不要把这种公开请求一概视为错误，必须结合当前身份规则、胜利条件和公开收益判断。
